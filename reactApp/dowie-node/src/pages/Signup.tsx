...
const Signup = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(`${backendUrl}/api/v1/account-signup`, {
        method: "POST",
        headers: {
      ...
      if (response.ok && data?.StatusCode === 200) {
        ...
        const errorMessage =
          data?.Data || data?.Message || `Signup failed (${response.status})`;
    ...
    <div className="min-h-screen bg-background bg-grid flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground text-sm">Start running Monte Carlo simulations today</p>
        </div>
